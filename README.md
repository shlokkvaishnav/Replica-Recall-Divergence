...
1. **Cross-system replication** (highest priority) — partly done, and the remaining part is sharper than it was. A 5-seed Qdrant sweep ([`research/cross_system_replication/`](research/cross_system_replication/)) established `completeness`/`e2e_recall` divergence under the same protocol. The **graph-quality axis is untested**: the `index_recall` comparison that appeared to show Qdrant differing from nano-db was taken over a corpus that was not HNSW-indexed for most or all of the measurement window ([`research/qdrant_optimizer_masking/`](research/qdrant_optimizer_masking/)), so it measured exact scans, not a graph. Two next steps: re-measure Qdrant's `index_recall` with indexing front-loaded before the baseline period, and point the protocol at a system with real anti-entropy (Weaviate).
