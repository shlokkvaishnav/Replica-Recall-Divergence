...
> That a ground-truth-free peer-agreement statistic (`loo_agreement`) can identify the degraded replica above chance, making it usable as a production detector for a failure mode that is currently invisible. That this failure mode generalizes beyond this one implementation. One specific objection to the detector has now been tested and does not hold: its above-chance performance is **not** an artifact of the harness's pinned, seeded query set. Three independent 5-seed conditions — pinned, non-pinned at 100 queries/round, non-pinned at 15 — all landed in the same 0.81–0.87 hit-rate band (chance = 1/3), with all nine pairwise between-condition comparisons non-significant (p = 0.15–0.90); see [`research/loo_agreement_nonpinned_queries/SPEC.md`](research/loo_agreement_nonpinned_queries/SPEC.md). Tested on SIFT1M, this topology, at 15–100 queries per round. That removes a confound; it does not by itself move this out of HYPOTHESIS, which still rests on one system and one implementation.
...
5. **Detector robustness against a different query *distribution*** — the pinning and query-count axes are now closed (see HYPOTHESIS above): removing pinning, and cutting to 15 queries per round, both left detection accuracy unchanged. What remains untested is a query workload drawn from a genuinely different distribution than the corpus's own — or an adversarial one.
