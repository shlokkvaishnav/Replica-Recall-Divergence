...
| Indexing gate so Qdrant's `index_recall` measures a graph (instrument for README priority #1) | [`qdrant_index_gate/`](qdrant_index_gate/) | Method (pre-registered, #28) | **In progress** — `--index-gate` blocks the baseline clock until every replica reports the corpus HNSW-indexed; `--indexing-threshold-kb` exposes Qdrant's segment threshold. First spec under `SPEC_TEMPLATE.md`'s instrument-characterization requirement |
