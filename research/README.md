...
| Chaos loop dies silently on a hung Docker call (harness defect) | [`qdrant_chaos_loop_timeout/`](qdrant_chaos_loop_timeout/) | Method (#38) | **In progress** — the loops run in daemon threads, so a `TimeoutExpired` from `subprocess.run(timeout=30)` ended them with no event and no log; a failed kill is now an event, `run_meta` carries `kill_count`/`chaos_no_kills`/`chaos_realized_s`, and `qdrant_sweep.py` refuses a chaos run with no kills |
