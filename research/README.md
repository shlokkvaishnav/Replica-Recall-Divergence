...
| Does the killed replica's `index_recall` on Qdrant heal after chaos stops? | [`qdrant_index_recall_healing/`](qdrant_index_recall_healing/) | Confirmatory (pre-registered, #35) | **In progress** — #31's protocol with a 180s quiesce (≈36 post-chaos samples vs 4–5), 5 new seeds; last-60s worst-replica `index_recall` vs the seed's own baseline range, time-to-baseline, and whether recovery tracks the tail re-indexing |
