...
| Does chaos degrade Qdrant's `index_recall` on a gated (indexed) corpus? | [`qdrant_gated_index_recall/`](qdrant_gated_index_recall/) | Confirmatory (pre-registered, #30) | **In progress** — re-runs PR #6's 5-seed sweep with #28's gate at 1,000 KB, new seeds, each sample conditioned on its replica's indexed fraction; run 0 is the before/after-gate `index_recall` spot-check #29 owed |
