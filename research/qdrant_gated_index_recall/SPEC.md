...
**Temporal resolution of the conditioning (review round 1).** The indexed fraction attached to each sample is the *nearest* telemetry poll, ~5s away, while a restarted node re-indexes its tail in 5–17s (#29) — so a sample inside that window could be tagged ≥0.95 by a neighbouring poll. Re-running the primary with a stricter join, both bracketing polls ≥0.95, gives baseline 0.9904, 0.9922, 0.9914, 0.9910, 0.9876 vs chaos 0.9875, 0.9600, 0.9810, 0.9768, 0.9825 — every chaos seed still below every baseline seed, p = 0.0079 — with chaos rounds kept 0.92–1.00 and baseline 0.27–1.00 (`analyze_gated.py --join bracket`). One baseline seed keeps 27% of its rounds, which by this spec's own Confounds rule makes that arm under-powered under the strict join — so this is a robustness check on the pre-registered metric, not a replacement for it. Read that way: the separation is not an artifact of the join's resolution; the tail was tested at the resolution the instrument actually has.
